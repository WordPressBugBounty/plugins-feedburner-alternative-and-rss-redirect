...
class sfmRedirectActions
{
    ...
    public function sfmActivateRedirect()
    {
		...
		if(!empty($_POST['rtype']))
		{
            ...
            //var_dump($web_url);
				...
				// Sience 4.1.0
				$respons->feed_url = $this->fixFeedUrl($respons->feed_url);
	...
	/**
	 * Fix feed url for new migration on follow.it
	 * make the feed url end with /rss if it is not
	 * @param string $feed_url
	 * @return string $new_feed_url
	 */
	public function fixFeedUrl($feed_url){
		$new_feed_url = $feed_url;
		if( strpos($feed_url, '/rss') === false ){
			$new_feed_url = $feed_url . '/rss';
		}
		return $new_feed_url;
	}
}
